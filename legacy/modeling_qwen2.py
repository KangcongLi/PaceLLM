...
class ActivationMemoryBank:
    def __init__(self, bank_size, embed_dim, device, theta_high=0.75, theta_low=0.25):
        self.bank_size = bank_size
        self.embed_dim = embed_dim
        self.device = device

        self.memory_keys = torch.zeros((bank_size, embed_dim), device=device)
        self.memory_values = torch.zeros((bank_size, embed_dim), device=device)
        self.usage_counts = torch.zeros(bank_size, device=device)
        self.theta_high = theta_high
        self.theta_low = theta_low

    def retrieve(self, queries, topk=5, negk=2, chunk_size=1024):
        """批量检索（分块处理防止显存溢出）"""
        retrieved = []
        for i in range(0, queries.size(0), chunk_size):
            chunk = queries[i:i+chunk_size]
            # chunk = chunk.to(self.memory_keys.device)

            # 计算余弦相似度
            sim = F.cosine_similarity(
                chunk.unsqueeze(1),  # [B, 1, D]
                self.memory_keys.unsqueeze(0),  # [1, M, D]
                dim=-1
            )  # [B, M]

            # 获取TopK和负样本
            topk_sim, topk_idx = sim.topk(topk, dim=-1)  # [B, K]
            topk_vals = self.memory_values[topk_idx]      # [B, K, D]
            
            neg_idx = sim.topk(negk, dim=-1, largest=False).indices  # [B, NK]
            neg_vals = self.memory_values[neg_idx]                   # [B, NK, D]

            # 向量化条件判断
            max_sim = topk_sim.max(dim=-1).values  # [B]
            mask_high = max_sim > self.theta_high
            mask_mid = (max_sim > self.theta_low) & ~mask_high
            mask_low = ~mask_high & ~mask_mid

            # 计算各情况结果
            pos_mean = topk_vals.mean(dim=1)  # [B, D]
            neg_mean = neg_vals.mean(dim=1)   # [B, D]
            
            result = torch.zeros_like(chunk)
            result[mask_high] = 1.0 * pos_mean[mask_high] + 0.1 * neg_mean[mask_high]
            result[mask_mid] = 0.5 * pos_mean[mask_mid] + 0.1 * neg_mean[mask_mid]
            result[mask_low] = chunk[mask_low]
            
            retrieved.append(result)

        return torch.cat(retrieved, dim=0)

    def store(self, keys, values, chunk_size=1024):
        """批量存储（分块处理+向量化更新）"""
        for i in range(0, keys.size(0), chunk_size):
            chunk_keys = keys[i:i+chunk_size]
            chunk_values = values[i:i+chunk_size]

            # 计算相似度
            sim = F.cosine_similarity(
                chunk_keys.unsqueeze(1),  # [B, 1, D]
                self.memory_keys.unsqueeze(0),  # [1, M, D]
                dim=-1
            )  # [B, M]

            # 获取Top5相似度
            topk_sim, topk_idx = sim.topk(5, dim=-1)  # [B, 5]
            mean_sim = topk_sim.mean(dim=-1)          # [B]

            # 分类处理
            mask_high = mean_sim > self.theta_high
            mask_mid = (mean_sim > self.theta_low) & ~mask_high
            mask_low = ~mask_high & ~mask_mid

            # 处理中等相似度样本
            if mask_mid.any():
                mid_keys = chunk_keys[mask_mid]
                mid_values = chunk_values[mask_mid]
                mid_topk = topk_idx[mask_mid]  # [N_mid, 5]
                
                # 展平所有索引并去重
                unique_idx, counts = torch.unique(mid_topk, return_counts=True)
                if unique_idx.numel() > 0:
                    # 计算平均更新量
                    avg_key = mid_keys.mean(dim=0)
                    avg_value = mid_values.mean(dim=0)
                    
                    # 加权更新
                    self.memory_keys[unique_idx] = 0.5 * self.memory_keys[unique_idx] + 0.5 * avg_key
                    self.memory_values[unique_idx] = 0.5 * self.memory_values[unique_idx] + 0.5 * avg_value
                    self.usage_counts[unique_idx] += 1

            # 处理低相似度样本
            if mask_low.any():
                low_keys = chunk_keys[mask_low]
                low_values = chunk_values[mask_low]
                
                # 找到最少使用的槽位
                n_replace = len(low_keys)
                replace_idx = torch.argsort(self.usage_counts)[:n_replace]
                
                # 循环使用槽位
                if n_replace > self.bank_size:
                    replace_idx = replace_idx.repeat(n_replace // self.bank_size + 1)[:n_replace]
                
                # 执行替换
                self.memory_keys[replace_idx] = low_keys[:len(replace_idx)]
                self.memory_values[replace_idx] = low_values[:len(replace_idx)]
                self.usage_counts[replace_idx] = 1


class MoEQwen2MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        ...
        self.memory_bank = ActivationMemoryBank(
            bank_size=100, 
            embed_dim=config.intermediate_size,
            device="cuda"
        )
    ...
    def forward(self, x):
        ...
        up = self.up_proj(x)
        inter = gate * up  # [B, L, D_ffn]

        # 展平处理
        inter_flat = inter.view(-1, inter.size(-1))  # [B*L, D_ffn]
        
        # 批量检索
        retrieved_flat = self.memory_bank.retrieve(inter_flat)
        # inter_flat = inter_flat.to(retrieved_flat.device)
        # 融合增强
        enhanced = 0.8 * inter_flat + 0.2 * retrieved_flat
        # inter_flat = inter_flat.to(enhanced.device)
        # 批量存储
        self.memory_bank.store(inter_flat.detach(), enhanced.detach())

        return self.down_proj(enhanced.view(B, L, -1))
...
class Qwen2DecoderLayer(nn.Module):
    def __init__(self, config: Qwen2Config, layer_idx: int):
        ...
        # if layer_idx == 13 or layer_idx == 26 or layer_idx == 14 or layer_idx == 25 or layer_idx == 12 or layer_idx == 24 or layer_idx == 15 or layer_idx == 23 or layer_idx == 11 or layer_idx == 22 or layer_idx == 10 or layer_idx == 21 or layer_idx == 9 or layer_idx == 20 or layer_idx == 8 or layer_idx == 19:
        #     self.mlp = MoEQwen2MLP(config)  # 只有这些层替换MLP
        # else:
        #     self.mlp = Qwen2MLP(config)  # 其他层继续普通MLP
        # if layer_idx == 26:
        #     self.mlp = MoEQwen2MLP(config)  # 只有这些层替换MLP
        # else:
        #     self.mlp = Qwen2MLP(config)  # 其他层继续普通MLP
        if layer_idx == 12 or layer_idx == 26:
            self.mlp = MoEQwen2MLP(config)  # 只有这些层替换MLP
        else:
            self.mlp = Qwen2MLP(config)  # 其他层继续普通MLP
        # self.mlp = Qwen2MLP(config)
        # self.mlp = MoEQwen2MLP(config)
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def __init__(self, config: Qwen2Config):
        ...
        # fixed_topk_indices_list = []
        # for layer_id in range(config.num_hidden_layers):
        #     path = os.path.join("/remote-home/likangcong/moe_neuron/cache_moe_model", f'layer{layer_id}_fixed_topk_indices.pth')
        #     fixed_topk_indices = torch.load(path).to(torch.device("cuda"))
